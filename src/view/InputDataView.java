...
public class InputDataView extends JFrame implements ActionListener {
	private InputDataListener ml;
	Graphics g;
	Graphics2D g2D;
	JPanel jPanel1;
	...
	public InputDataView() {
		ml = new InputDataListener(this);
		jPanel1 = new JPanel(new GridLayout(10, 1));
		jTextField = new JTextField("this is a field");
		jbtnAdd = new JButton();
		jbtnAdd.setIcon(new ImageIcon(
				Toolkit.getDefaultToolkit().createImage(InputDataView.class.getResource("/image/Add.png"))));
		jbtnMove = new JButton();
		jbtnMove.setSize(10, 10);
		jbtnMove.setIcon(new ImageIcon(
				Toolkit.getDefaultToolkit().createImage(InputDataView.class.getResource("/image/move.png"))));
		jbtnDelete = new JButton();
		jbtnDelete.setIcon(new ImageIcon(
				Toolkit.getDefaultToolkit().createImage(InputDataView.class.getResource("/image/delete.png"))));
	...
	public void paint(Graphics g) {
		...
		paint_lines(ml.getLines());
		paint_nodes(ml.getNodes(), ml.getNodeDragged(), ml.getNodeDraggedOffset());
		g2D.dispose();
	}
